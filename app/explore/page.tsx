...
export default async function ExplorePage() {
      ...
      <main className="flex min-h-screen items-center justify-center bg-black px-6 text-white">
        <div className="text-center">
          ...
          <p className="mt-4 text-red-400">
            Could not load cards: {cardsError.message}
          </p>
        </div>
      </main>
    )
  ...
  const exploreCards: ExploreCardType[] = shuffle(cards)
    .slice(0, 50)
    .map((card) => ({
      ...card,
      set_name:
        setNameById.get(card.set_id) ??
        card.set_id,
    }))

  const initialReactions: Record<
    string,
    'like' | 'love'
  > = {}

  if (user && exploreCards.length > 0) {
    const cardIds = exploreCards.map(
      (card) => card.id
    )
