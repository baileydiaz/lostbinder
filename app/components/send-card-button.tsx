...
export default function SendCardButton({ cardId, userId, friends }: Props) {
  ...
  async function sendCard() {
    ...
    const { error: sendError } = await supabase.from('card_shares').insert({
      sender_id: userId,
  ...
  return (
    <div className="relative">
      <button
        ...
        className="rounded-full border border-white/15 px-4 py-2.5 text-sm text-zinc-300 transition hover:border-white/40 hover:text-white"
      >
        ↗ Send to Friend
      </button>
      ...
      {open ? (
        <>
          <button
            type="button"
            aria-label="Close send card"
            onClick={closeMenu}
            className="fixed inset-0 z-40 bg-black/70 sm:hidden"
          />

          <div className="fixed inset-x-3 top-1/2 z-50 max-h-[80dvh] -translate-y-1/2 overflow-hidden rounded-2xl border border-white/10 bg-zinc-950 shadow-2xl sm:absolute sm:inset-x-auto sm:right-0 sm:top-full sm:mt-3 sm:w-[320px] sm:translate-y-0">
            {!selectedFriend ? (
              <>
                <div className="flex items-start justify-between border-b border-white/10 px-4 py-4">
                  <div>
                    <p className="text-sm font-semibold text-white">Send to a friend</p>
                    <p className="mt-1 text-xs text-zinc-600">Choose who you want to send this card to.</p>
                  </div>
                  <button type="button" onClick={closeMenu} className="text-zinc-600 hover:text-white">✕</button>
                </div>
                ...
                <div className="p-4">
                  {sent ? (
                    <div className="py-5 text-center">
                      ...
                      <p className="mt-3 font-semibold text-white">Card sent</p>
                      <p className="mt-1 text-sm text-zinc-600">
                        It will appear in @{selectedFriend.username || 'collector'}&apos;s Sent to You page.
                      </p>
                      <button
                        type="button"
                        onClick={closeMenu}
                    ...
                    <>
                      ...
                      />
                      <div className="mt-1 text-right text-[10px] text-zinc-700">{note.length}/200</div>
                      ...
                      >
                        {sending ? 'Sending...' : `Send to @${selectedFriend.username || 'collector'}`}
                      </button>
                    </>
                  )}
                </div>
